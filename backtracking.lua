...
do
  local get_unvisited = function(self, x, y)
    local dirs = {}
  ...
  local step = function(self)
    ...
    if get_unvisited(self, x, y) then
      dirs = get_unvisited(self, x, y)
      if #get_unvisited(self, x, y) > 1 then
      ...
      if #dir_stack > 0 then
        while #dir_stack > 0 do
          ...
          if get_unvisited(self, value.x, value.y) then
            x, y = value.x, value.y
            dirs = get_unvisited(self, x, y)
            break
          end
        ...
        return true -- if algorithm finished
      end
    end
  ...
  local iter = function(self)
    return function()
      if not self:step() then self._grid:set_cursor(self._x, self._y) end
    end
  end
